...
// Usage: node generateRandomMetadata.js
...
async function uploadImage(client, imagePath) {
  ...
  // Use store({ image }) instead of storeBlob for compatibility
  const metadata = await client.store({
    image: new File([imageData], path.basename(imagePath), { type: 'image/jpeg' }),
    name: path.basename(imagePath),
    description: 'Dangles NFT image upload'
  });
  // metadata.image is a File object with .href as the IPFS URL
